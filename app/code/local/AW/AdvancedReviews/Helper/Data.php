...
//    }
    ...
    public function updateReviewProduct($productId) {
        $arg_attribute = 'review_filter';
        $arg_value = '';
        $optionVal = array();
        ...
        //var_dump($product->getData());die('ddd');
        $summaryData = Mage::getModel('review/review_summary')
                        ->setStoreId($storeId)
                        ->load($productId);
        $summaryValue = $summaryData->getRatingSummary();//var_dump($summaryValue);//die('ddd');
        if($summaryValue){
            if($summaryValue == 0){
                $arg_value = '';
            }
            elseif($summaryValue > 0 && $summaryValue <= 20){
                $arg_value = $optionVal[0];
            }elseif($summaryValue > 20 && $summaryValue <= 40){
                $arg_value = $optionVal[1];
            }elseif($summaryValue > 40 && $summaryValue <= 60){
                $arg_value = $optionVal[2];    
            }elseif($summaryValue > 60 && $summaryValue <= 80){
                $arg_value = $optionVal[3];
            }else{
                $arg_value = $optionVal[4];
            }   
        }
        //var_dump($arg_value);die('32132');
        Mage::log('arg value : '. $arg_value);
        try { 
            $product->setReviewFilter($arg_value);
           // $product->getResource()->saveAttribute($product, $arg_attribute); 
            $product->setData($arg_attribute,$arg_value);

            $product->save();
        }catch(Exception $ex) { 
            Mage::log('Save product error at AW_AdvancedReviews_Helper_Data');
        }    
